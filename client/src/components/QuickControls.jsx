...
export function QuickControls({ flows, onRun }) {
  ...
  const findFlow = (matches) =>
    list.find(f => matches.some(m => (f.name || '').toLowerCase().includes(m)));
        ...
        {QUICK.map(({ match, label, Icon }) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
              aria-label={`Kjør ${label}${disabled ? ' (ikke konfigurert)' : ''}`}
              className={[
                'group flex items-center gap-2 rounded-xl border px-3 py-2.5 text-left transition-all',
                disabled
                  ? 'border-nx-line/40 bg-nx-panel/30 text-nx-mute opacity-60 cursor-not-allowed'
                  : 'border-nx-line/70 bg-nx-panel/60 hover:border-nx-cyan/60 hover:text-nx-cyan'
              ].join(' ')}
            >
              <Icon size={16} className="text-nx-cyan group-hover:scale-110 transition-transform" aria-hidden="true" />
